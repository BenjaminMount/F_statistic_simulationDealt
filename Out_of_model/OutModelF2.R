...
#封装函数 模拟第三个种群受到两个种群的影响
calculate_next_gen_num2 <- function(current_num, N, a1, b1, m1, p1, m2, p2) {
  # 计算当前代中等位基因的频率
  p0 <- current_num / (2 * N)
  # m1 和 p1 分别代表来自第一个种群的迁移率和等位基因频率
  # m2 和 p2 分别代表来自第二个种群的迁移率和等位基因频率
  # 假设两个种群的迁移是独立且同时发生的
...
#-----------三个种群都受到线性进化压力的影响，不同的是种群3的等位基因频率也会发生变化
...
F_simulation_modified <- function(N, K, a1, b1, m_ij, p0, p3, num_generations) {
  ...
  n_i[1, 1] <- 2 * N * p0 
  n_i[2, 1] <- 2 * N * p0
  n_i[3, 1] <- 2 * N * p3    # 第三个种群使用p3_initial初始化

  for (t in 1:(num_generations - 1)) {
    # 在每次循环开始时，先更新第三个种群的等位基因频率p3
    p1 = n_i[1 , t] / (2 * N)
    p2 = n_i[2 , t] / (2 * N)
    p3 = n_i[3, t] / (2 * N)
...
diag(m_ij) <- 1 - 0.005 
m_ij[3,3] <- 1 - 0.005*2
p0 <- 0.3   # 前两个种群的初始等位基因频率
p3 <- 0.2   # 第三个种群的固定等位基因频率
...
loess_fit_st <- loess(mean_diffs ~ generation_time, span = 0.3)  # 使用LOESS拟合
# 生成用于绘制拟合曲线的预测值
...
loess_fit_F2 <- loess(F2_values ~ generation_time, span = 0.3)  # 使用LOESS拟合
# 生成用于绘制拟合曲线的预测值
...
lines(pred_time_vals_st, pred_vals_st, col = "brown1", lwd = 2)  # 添加拟合曲线
